...
type helper = {target:{innerText:string}}
...
function Page() {
  ...
  const handleArts = (e: React.ChangeEvent&{target:{name:string,value:string}}) => {
    setArticle({ ...article, [e.target.name]: e.target.value })
  }
  const handleCategory = (e: React.MouseEvent<HTMLButtonElement>&helper) => {
    setArticle({ ...article, category: e.target.innerText })
  }
  const addTag = () => {
    const tagCollecter:HTMLCollectionOf<Element> = document.getElementsByClassName('tagMan')
    if(tagCollecter[0]){
      (tagCollecter[0] as HTMLElement).focus()
    }
  ...
  const deleteTag = (e:React.MouseEvent<HTMLParagraphElement>&helper) => {
  ...
  const handleImageTwo = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className={`${pop.className}`}>
      <form className="bg-white rounded leading-6 pb-[3em]  mx-auto pt-[2em] px-[1.5em]  w-[80%] h-[max-content]">
        ...
        <input id="tags" value={tag} onChange={(e) => setTag(e.target.value)} className="tagMan outline-none bg-slate-50 rounded-md block placeholder:text-sm w-full px-[1.3em] mt-[1em] py-[1em]" placeholder="tags e.g web development" type="text" />
